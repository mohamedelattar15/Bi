...
\begin{titlepage}
  ...
  \begin{minipage}[c]{0.30\linewidth}
    \raggedright
    \includegraphics[width=3.25cm]{images/logo_uae.png}
  \end{minipage}
  \hfill
  ...
  \begin{minipage}[c]{0.30\linewidth}
    \raggedleft
    \includegraphics[width=3.25cm]{images/logo_fstt.png}
  \end{minipage}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\textwidth,height=0.66\textheight,keepaspectratio]{images/demarche_de_projet.png}
...
Le projet s'appuie sur le jeu de données \textit{Grocery Sales Database}, publié sur Kaggle par l'utilisateur \cite{andrexibiza}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=14cm,keepaspectratio]{images/diagaram_de architecture .png}
\caption{architecture logicielle simplifiée}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=20cm,keepaspectratio]{images/etl_diagram_detailled.png}
\caption{ETL Pipeline Diagram détaillé}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=10cm,keepaspectratio]{images/data_model.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=18cm,keepaspectratio]{images/Sequence Diagram.png}
...
\begin{longtable}{>{\ttfamily}p{5cm}cp{7cm}}
...
KPIs principaux (chiffre d'affaires, quantités vendues, nombre de transactions et panier moyen). \\

/api/sales/over-time & GET &
Évolution temporelle du chiffre d'affaires. \\

/api/sales/by-category & GET &
Répartition des ventes par catégorie de produits. \\

/api/sales/by-city & GET &
Distribution géographique des ventes par ville. \\

/api/products/all & GET &
Liste complète des produits avec indicateurs de performance. \\
...
/api/products/price-
volume-matrix & GET &
Matrice d'analyse prix-volume des produits. \\

/api/customers/segments & GET &
Segmentation des clients (VIP, Régulier, Occasionnel, etc.). \\

/api/customers/top & GET &
Classement des meilleurs clients selon le chiffre d'affaires généré. \\

/api/employees/top & GET &
Classement des employés les plus performants. \\

/api/employees/perform-
ance-by-age & GET &
Analyse de la performance selon les tranches d'âge. \\

/api/basket/analysis & GET &
Résultats de l'analyse du panier d'achat et règles d'association. \\

/api/insights/revenue-
concentration & GET &
Mesure de la concentration du chiffre d'affaires. \\
...
rfm & GET &
Segmentation client basée sur le modèle RFM. \\

/api/filters & GET &
Options de filtrage disponibles (dates, catégories, villes, etc.). \\
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=20cm,keepaspectratio]{images/sale_performance.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=12cm,keepaspectratio]{images/product_performance.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=12cm,keepaspectratio]{images/customer_performance.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=12cm,keepaspectratio]{images/employee_performance.png}
...
La page d'analyse du panier est consacrée aux règles d'association entre produits (Market Basket Analysis). Elle exploite une table pré-calculée \texttt{basket\_analysis\_results} contenant environ 75~020 règles d'association, générée par le script \texttt{generate\_basket\_table.py} à partir du fichier \texttt{groceries\_long.csv}. Cette approche par pré-calcul, alignée sur la méthodologie Power BI DAX, garantit des temps de réponse instantanés sans requête complexe à l'exécution.

\begin{infobox}{Contrainte du jeu de données}
Chaque transaction contient exactement un seul produit, ce qui implique que 98~\% des paniers (regroupés par client + date) ne contiennent qu'un article. Les valeurs de lift sont donc toutes inférieures à 1,0 (maximum observé~: ~0,36). Les seuils par défaut sont fixés très bas (\texttt{min\_support~=~0,000001}, \texttt{min\_lift~=~0,0}) pour capturer les meilleures associations disponibles.
\end{infobox}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=12cm,keepaspectratio]{images/basket_analysis.png}
...
La page affiche quatre métriques de haut niveau~:
...
\toprule
\rowcolor{TableHeader}
\color{white}KPI & \color{white}Description \\
\midrule
Paniers analysés & Nombre total de paniers (regroupement client + date). \\
Produits analysés & Nombre de produits distincts présents dans les règles. \\
Règles d'association & Nombre de règles correspondant aux seuils courants. \\
Produit avec le plus de connexions & Produit central du réseau d'associations avec son nombre de connexions. \\
\bottomrule
\end{tabularx}
\caption{Indicateurs clés de la page analyse du panier}
\end{table}

\subsection{Contrôles interactifs}

Deux curseurs permettent d'ajuster la sensibilité de l'analyse~:

\begin{itemize}
  \item \textbf{Support minimum} (0,0005~\% à 1~\%)~: seuil de fréquence d'apparition d'une paire.
  \item \textbf{Lift minimum} (0,0 à 0,4)~: seuil de force de l'association.
\end{itemize}

\subsection{Visualisations}

\begin{itemize}
  \item \textbf{Distribution du lift}~: Histogramme montrant la répartition des 75~020 règles par tranche de lift. Environ 48~\% des règles ont un lift inférieur à 0,05, et seulement 0,1~\% dépassent 0,20. Ce graphique sert d'indicateur de qualité des données.
  
  \item \textbf{Produits hubs}~: Diagramme en barres horizontal classant les produits ayant le plus grand nombre de connexions avec d'autres produits. Le produit le plus connecté (ex. \textit{Chicken -- Wieners}) peut atteindre jusqu'à 361 connexions. Ces produits sont les meilleurs candidats pour des actions de vente croisée.
  
  \item \textbf{Graphe de connaissances -- Liens produits}~: Un réseau SVG interactif où chaque nœud représente un produit (taille proportionnelle au nombre de connexions), et chaque arête représente une association (épaisseur proportionnelle au lift). Les cinq produits hubs principaux sont disposés en cercle, reliés à leurs meilleurs associés. La couleur distingue les groupes de hubs.
  
  \item \textbf{Top 10 paires de produits (par lift)}~: Diagramme en barres horizontal présentant les dix associations dont le lift est le plus élevé, permettant d'identifier rapidement les paires les plus notables.
  
  \item \textbf{Affinités par catégorie}~: Tableau des associations entre catégories de produits (ex. \textit{Confections × Confections~:~1~168 paires}), avec le lift moyen et le nombre de paires.
  
  \item \textbf{Tableau des règles d'association}~: Tableau complet listant toutes les paires avec les métriques de support, confiance (dans les deux directions) et lift. Chaque règle est accompagnée d'un badge de force (\textit{faible}, \textit{moyenne}, \textit{forte}) selon la valeur du lift.
\end{itemize}
