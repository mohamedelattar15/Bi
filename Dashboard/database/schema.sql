...
CREATE TABLE dim_customer (
    ...
    full_name VARCHAR(250) GENERATED ALWAYS AS (
        TRIM(CONCAT(customerfirstname, ' ', COALESCE(middleinitial || ' ', ''), customerlastname))
    ) STORED,
    address VARCHAR(200),
    cityid INTEGER,
    city VARCHAR(100),
...
CREATE TABLE dim_employee (
    ...
    full_name VARCHAR(200) GENERATED ALWAYS AS (
        TRIM(CONCAT(employeefirstname, ' ', employeelastname))
    ) STORED,
    birthdate DATE,
    gender VARCHAR(20),
    hiredate DATE,
    city VARCHAR(100),
    cityid INTEGER,
    age_at_hire INTEGER GENERATED ALWAYS AS (
        EXTRACT(YEAR FROM age(hiredate, birthdate))::INTEGER
    ) STORED,
    years_of_service INTEGER GENERATED ALWAYS AS (
        EXTRACT(YEAR FROM age(CURRENT_DATE, hiredate))::INTEGER
    ) STORED,
    ...
    seniority_group VARCHAR(20) GENERATED ALWAYS AS (
        CASE 
            WHEN EXTRACT(YEAR FROM age(CURRENT_DATE, hiredate)) < 1 THEN 'New'
            WHEN EXTRACT(YEAR FROM age(CURRENT_DATE, hiredate)) BETWEEN 1 AND 5 THEN 'Confirmed'
            ELSE 'Senior'
        END
    ) STORED
);
...
CREATE INDEX idx_dim_employee_age_group ON dim_employee(age_group);
CREATE INDEX idx_dim_employee_seniority ON dim_employee(seniority_group);
...
GROUP BY 
    ...
    s.employeeid, e.gender, e.age_group, e.seniority_group,
...
GROUP BY 
    ...
    e.gender, e.age_group, e.seniority_group,
    c.country;

CREATE UNIQUE INDEX idx_mv_monthly_sales_unique 
ON mv_monthly_sales(year, month, categoryid, gender, age_group, country);
...
FROM fact_sales s
JOIN dim_employee e ON s.employeeid = e.employeeid
GROUP BY e.employeeid, e.full_name, e.gender, e.age_group, e.seniority_group, e.city;

CREATE UNIQUE INDEX idx_mv_employee_perf ON mv_employee_performance(employeeid);
CREATE INDEX idx_mv_employee_perf_age_group ON mv_employee_performance(age_group);
CREATE INDEX idx_mv_employee_perf_seniority ON mv_employee_performance(seniority_group);
